  ...
  methods: {
    ...
    spawnApproach() {
      ...
      if (this.currentAircraftCount >= this.currentMaxAircraftCount) {
        console.log(`已达到${this.difficulty}难度下的最大飞机数量限制: ${this.currentMaxAircraftCount}`);
        return;
      }
            ...
            if (plane.targetRunwayX !== undefined && plane.targetRunwayY !== undefined) {
              ...
              if (distToRunway < 20) {
                ...
                const message = this.isEnglish
                  ? `${plane.id}, landing runway ${plane.landingRunway}`
                  : `${plane.id}, 正在着陆跑道 ${plane.landingRunway}`;
                this.addToCommunicationLog(message);
                
                console.log(`${plane.id} 到达跑道入口，开始着陆`);
    ...
    onDrag(event) {
      ...
      const newY = (event.clientY - rect.top) * scaleY;
        ...
        // 保存拖拽中的指令线信息
          ...
          // 添加跑道相关信息
          isNearRunwayEntrance: isNearRunwayEntrance.isNear,
          runwayInfo: isNearRunwayEntrance.isNear ? isNearRunwayEntrance.info : null
        };
      }
    },
    endDrag(event) {
    ...
    updateApproachFlight(plane) {
      try {
        ...
        if (!plane || !plane.approachPath) {
          console.log("updateApproachFlight: 无效的飞机或路径");
          return false;
        }
    ...
    checkAndSpawnNewAircraft() {
      ...
      console.log(`检查飞机数量: 当前 ${this.airplanes.length}/${this.currentMaxAircraftCount}`);
      ...
      if (this.airplanes.length < this.currentMaxAircraftCount) {
        ...
        console.log(`当前飞机数量(${this.airplanes.length})低于最大限制(${this.currentMaxAircraftCount})，生成新飞机`);
      }
    },
  },
  watch: {
