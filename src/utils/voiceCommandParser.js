...
function speedTokensToNumber(tokens) {
  const digitSpeed = headingTokensToDigits(tokens);
  const speed = Number.parseInt(digitSpeed, 10);
  return Number.isFinite(speed) && speed >= 60 && speed <= 400 ? speed : null;
}
...
export function normalizeVoiceCommand(command) {
  ...
  normalized = normalized.replace(
    new RegExp(`^(?:flight|aircraft)?\\s*((?:(?:\\d|${digitAlternation}|${Object.keys(NUMBER_WORDS).join("|")})\\s*){2,8})(?=\\s+(?:turn|heading|head|land|take|climb|descend|maintain|reduce|increase|speed|go|proceed|direct|fly|cleared))`, "i"),
  ...
  normalized = normalized.replace(
    new RegExp(`\\b((?:maintain|reduce|increase)\\s+speed|speed)\\s+(?:to\\s+)?((?:(?:\\d+|${digitAlternation}|${Object.keys(NUMBER_WORDS).join("|")}|hundred)\\s*){1,4})(?:\\s*knots?)?\\b`, "gi"),
    (match, prefix, speedPhrase) => {
      const speed = speedTokensToNumber(normalizeWhitespace(speedPhrase).split(" "));
      return speed ? `${prefix} ${speed} knots` : match;
    },
  );
...
export function extractSpeed(command) {
  const speedMatch = command.match(/\b(?:(?:maintain|reduce|increase)\s+speed|speed)\s+(?:to\s+)?(\d{2,3})(?:\s+knots?)?\b/i);
  if (!speedMatch) return null;
  const speed = Number.parseInt(speedMatch[1], 10);
  if (!Number.isFinite(speed) || speed < 130 || speed > 320) return null;
